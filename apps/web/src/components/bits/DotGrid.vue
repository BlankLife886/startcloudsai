<template>
  <section :class="`dot-grid-shell ${className}`" :style="style">
...
import { nextTick, onMounted, onUnmounted, computed, ref, useTemplateRef, watch } from 'vue'
...
const onMove = (event: MouseEvent) => {
  ...
  const dt = pointerState.lastTime ? now - pointerState.lastTime : 16
  ...
  for (const dot of dots.value) {
    const dist = Math.hypot(dot.cx - pointerState.x, dot.cy - pointerState.y)
    if (speed > props.speedTrigger && dist < props.proximity && !dot._inertiaApplied) {
      dot._inertiaApplied = true
      gsap.killTweensOf(dot)
      const pushX = dot.cx - pointerState.x + vx * 0.005
      const pushY = dot.cy - pointerState.y + vy * 0.005
      gsap.to(dot, {
