  ...
  >
    ...
    >
      ...
      >
        ...
        >
          <div
            ...
            :data-src="item.src"
            :data-background-src="item.backgroundSrc || item.src"
          ...
          >
            <div
              class="dome-gallery-tile absolute block inset-[10px] bg-transparent overflow-hidden transition-transform duration-300 cursor-pointer pointer-events-auto transform translate-z-0 focus:outline-none"
              role="button"
              tabindex="0"
            ...
            >
              <div class="dome-gallery-tile-placeholder" aria-hidden="true" />
              <img
                :src="item.src"
                draggable="false"
                :alt="item.alt"
                class="dome-gallery-tile-image w-full h-full object-cover pointer-events-none"
                loading="lazy"
                decoding="async"
                :style="{
                  backfaceVisibility: 'hidden',
      ...
      <div
        ...
        :style="{
          backgroundImage:
            'radial-gradient(rgba(235, 235, 235, 0) 72%, var(--overlay-blur-color, #060010) 100%)',
        }"
      />
      <div
        class="absolute inset-0 m-auto z-[3] pointer-events-none"
        :style="{
          WebkitMaskImage:
            'radial-gradient(rgba(235, 235, 235, 0) 76%, var(--overlay-blur-color, #060010) 94%)',
          maskImage:
            'radial-gradient(rgba(235, 235, 235, 0) 76%, var(--overlay-blur-color, #060010) 94%)',
        }"
      />
      <div
        class="absolute left-0 right-0 h-[88px] z-[5] pointer-events-none top-0 rotate-180"
        :style="{
          background: 'linear-gradient(to bottom, transparent, var(--overlay-blur-color, #060010))',
        }"
      />
      <div
        class="absolute left-0 right-0 h-[88px] z-[5] pointer-events-none bottom-0"
        :style="{
          background: 'linear-gradient(to bottom, transparent, var(--overlay-blur-color, #060010))',
        }"
      />
      ...
      >
        ...
        <div
          ref="frameRef"
          class="dome-gallery-frame h-full aspect-square flex"
...
const emit = defineEmits<{
  (event: 'select-image', image: ImageItem): void
}>()
...
const isVisibleInViewport = ref(true)
...
const onTileImageLoad = (e: Event) => {
  const img = e.currentTarget as HTMLImageElement | null
  img?.closest('.dome-gallery-tile')?.classList.add('is-loaded')
...
function buildItems(pool: (string | ImageItem)[], seg: number) {
  ...
  const normalizedImages = pool.map((image) => {
    ...
    return {
      src: image.src || '',
      alt: image.alt || '',
      backgroundSrc: image.backgroundSrc || image.src || '',
    }
  })
  ...
  return coords.map((c, i) => ({
    ...
    backgroundSrc: usedImages[i].backgroundSrc,
  }))
}
...
const activateTile = (e: Event) => {
  if (isDragging.value) return
  if (performance.now() - lastDragEndAt.value < 80) return
  if (isOpening.value) return

  if (props.clickMode === 'select') {
    if (performance.now() - lastSelectAt.value < 160) return
    lastSelectAt.value = performance.now()
    const el = e.currentTarget as HTMLElement
    const parent = el.parentElement
    emit('select-image', {
      src: parent?.dataset.src || el.querySelector('img')?.src || '',
      alt: el.querySelector('img')?.alt || '',
      backgroundSrc:
        parent?.dataset.backgroundSrc || parent?.dataset.src || el.querySelector('img')?.src || '',
    })
    return
  }
...
onMounted(() => {
  ...
  if (typeof IntersectionObserver !== 'undefined') {
    visibilityObserver = new IntersectionObserver(
      ([entry]) => {
        isVisibleInViewport.value = entry?.isIntersecting ?? true
      },
      { threshold: 0.08 },
    )
...
<style scoped src="./DomeGallery.css"></style>
