...
const props = defineProps({
  colors: {
    type: Array,
    default: () => ['#6ff7d2', '#5fb8ff', '#ff6b78', '#ffd45f', '#f7f7f2'],
  },
...
function setupCanvas() {
  ...
  function draw(time) {
    ...
    for (let strand = 0; strand < strandCount; strand += 1) {
      ...
      const amplitude = height * (0.055 + strand * 0.006)
      const offset = (strand - (strandCount - 1) / 2) * 16
      ...
      const tracePath = () => {
        ...
        for (let x = -step; x <= width + step; x += step) {
          ...
          const wave =
            Math.sin(x * frequency + time * props.speed * (1.6 + strand * 0.16) + phase) * 0.66 +
            Math.sin(x * frequency * 1.8 - time * props.speed * 1.15 + phase * 0.7) * 0.34
...
<template>
  <div class="strands-band" aria-hidden="true">
