...
const props = defineProps({
  ...
  speed: { type: Number, default: 0.32 },
})
...
void main() {
  ...
  vec2 pointer = (uPointer - 0.5) * vec2(0.18, 0.1);
  float beamY = -0.08 + pointer.y + sin(p.x * 2.2 + t * 0.55) * 0.012;
  ...
  float columnX = 0.14 + pointer.x + sin(t * 0.43) * 0.02;
  ...
  vec3 color = uColor * (softBeam * 0.38 + beamCore * 1.25) * beamGate;
  color += uColor * (columnGlow * 0.28 + columnCore * 1.35) * columnGate;
  color += mix(uColor, uAccent, 0.55) * flare * 1.15;
  color += uColor * fog * 0.18;
  color += uAccent * wisps * 0.42;

  float edgeFade = (1.0 - smoothstep(0.1, 0.74, abs(p.x)))
    * (1.0 - smoothstep(0.16, 0.64, abs(p.y)));
  color *= 0.46 + edgeFade * 0.82;
...
function setupScene() {
  ...
  function render(now) {
    ...
    pointerCurrent.lerp(pointerTarget, 1 - Math.exp(-delta * 5.8))
...
<template>
  <div ref="mountRef" class="laser-flow-hero" :class="{ 'is-unavailable': unavailable }"></div>
</template>
