import { ArrowLeft, ArrowRight, BookOpen, CheckSquare, ClipboardPaste, Download, Eraser, FolderPlus, History, ImagePlus, LoaderCircle, PenLine, Plus, SlidersHorizontal, Sparkles, Trash2, Upload } from "lucide-react";
...
import { requestCanvasBackgroundRemoval } from "@/services/canvas-task-api";
import { fetchSiteBackgroundRemovalTools, type SiteBackgroundRemovalTool } from "@/services/site-model-catalog";
...
export default function ImagePage() {
    ...
    const [backgroundRemovalTools, setBackgroundRemovalTools] = useState<SiteBackgroundRemovalTool[]>([]);
    const [backgroundRemovalBusyId, setBackgroundRemovalBusyId] = useState("");
    ...
    useEffect(() => {
        void refreshLogs();
        void fetchSiteBackgroundRemovalTools().then(setBackgroundRemovalTools).catch(() => setBackgroundRemovalTools([]));
    }, []);

    const backgroundRemovalTool = backgroundRemovalTools.find((tool) => tool.default) || backgroundRemovalTools[0];
    ...
    const removeResultBackground = async (image: GeneratedImage, index: number) => {
        if (!backgroundRemovalTool || backgroundRemovalBusyId) return;
        const started = performance.now();
        setBackgroundRemovalBusyId(image.id);
        try {
            const removed = await requestCanvasBackgroundRemoval(
                { id: image.id, name: `result-${index + 1}.png`, type: image.mimeType || "image/png", dataUrl: image.dataUrl, storageKey: image.storageKey },
                backgroundRemovalTool.id,
            );
            ...
            const processed: GeneratedImage = {
                ...
                width: meta.width,
                height: meta.height,
                bytes: getDataUrlByteSize(removed.dataUrl),
                mimeType: removed.type,
            };
            setResults((current) => [...current, { id: nanoid(), status: "success", image: processed }]);
            message.success("背景已移除，原图已保留");
        } catch (error) {
            message.error(error instanceof Error ? error.message : "背景移除失败");
        } finally {
            setBackgroundRemovalBusyId("");
        }
    };
                                ...
                                {results.map((result, index) =>
                                    result.status === "success" && result.image ? (
                                        <ResultImageCard
                                            ...
                                            onRemoveBackground={backgroundRemovalTool ? removeResultBackground : undefined}
                                            removingBackground={backgroundRemovalBusyId === result.image.id}
                                        />
...
function ResultImageCard({
    ...
    onRemoveBackground?: (image: GeneratedImage, index: number) => void;
    ...
    return (
        <div className="overflow-hidden rounded-lg border border-stone-200 bg-background dark:border-stone-800">
            ...
            <div className="space-y-2 border-t border-stone-200 px-3 py-2.5 dark:border-stone-800">
                ...
                <div className={`grid min-w-0 gap-2 ${onRemoveBackground ? "grid-cols-4" : "grid-cols-3"}`}>
                    {onRemoveBackground ? (
                        <Tooltip title="移除背景">
                            <Button className={RESULT_ACTION_BUTTON_CLASS} size="small" icon={<Eraser className="size-3.5" />} loading={removingBackground} disabled={removingBackground} onClick={() => void onRemoveBackground(image, index)}>
                                移除背景
                            </Button>
                        </Tooltip>
                    ) : null}
