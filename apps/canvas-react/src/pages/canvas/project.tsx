...
function InfiniteCanvasPage() {
    ...
    useEffect(() => {
        void fetchSiteBackgroundRemovalTools()
            .then((tools) => setBackgroundRemovalModelKey((tools.find((tool) => tool.default) || tools[0])?.id || ""))
            .catch(() => setBackgroundRemovalModelKey(""));
    }, []);
    ...
    const removeBackgroundFromImageNode = useCallback(
        async (node: CanvasNodeData) => {
            ...
            const controller = startGenerationRequest(childId, node.id, childId);
            try {
                const removed = await requestCanvasBackgroundRemoval(source, backgroundRemovalModelKey, controller.signal);
                const meta = await readImageMeta(removed.dataUrl);
                const size = fitNodeSize(meta.width, meta.height, node.width, node.height);
                setNodes((prev) =>
                    prev.map((item) =>
                        item.id === childId
                            ? {
        ...
        [backgroundRemovalModelKey, finishGenerationRequest, message, startGenerationRequest],
    );
