    ...
    return (
        <div className="flex min-w-0 items-center gap-1">
            <Select value={model} onValueChange={onModelChange}>
                <SelectTrigger
                    className="h-9 min-w-0 max-w-36 rounded-full border-0 bg-transparent px-2.5 text-xs font-medium shadow-none hover:bg-black/5 focus:ring-0 dark:bg-transparent dark:hover:bg-white/10"
                    title={current.displayName || current.model}
                ...
                <SelectContent data-canvas-no-zoom position="popper" side="top" align="start" sideOffset={6} className="z-[1200] min-w-32 rounded-xl border border-border/70 bg-popover p-1 shadow-xl">
                    {current.supportedReasoningEfforts.map((item) => (
                        <SelectItem key={item.reasoningEffort} value={item.reasoningEffort}>
                            {effortLabels[item.reasoningEffort]}
                        </SelectItem>
                    ))}
                </SelectContent>
            </Select>
...
function PermissionModeMenu({ permissionMode, theme, onChange }: { permissionMode: AgentPermissionMode; theme: (typeof canvasThemes)[keyof typeof canvasThemes]; onChange: (permissionMode: AgentPermissionMode) => void }) {
    ...
    return (
        ...
        >
            <button
                type="button"
                className="flex h-9 shrink-0 items-center gap-1.5 rounded-full px-2.5 text-xs font-medium transition hover:bg-black/5 dark:hover:bg-white/10"
                style={{ color: permissionMode === "full" ? "#ea580c" : theme.node.text }}
                aria-label="选择 Codex 权限模式"
            >
                {current.icon}
