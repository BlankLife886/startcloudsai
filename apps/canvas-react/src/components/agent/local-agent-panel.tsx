...
export function LocalAgentPanel({ embedded, headless, autoConnect }: { embedded?: boolean; headless?: boolean; autoConnect?: boolean }) {
    ...
    useEffect(() => {
        ...
        source.addEventListener("chat_message", (event) => {
            ...
            enqueueEvent(() => {
                ...
                const pending =
                    data.message!.role === "user" ? [...current.messages].reverse().find((item) => item.role === "user" && item.threadId === threadId && !item.turnId && (!clientMessageId || item.clientMessageId === clientMessageId)) : undefined;
                ...
                if (next.role === "user" && !next.attachments?.length) {
                    void readAgentUserMessages(threadId)
                        .then((storedMessages) => {
    ...
    const resumeThread = async (threadId: string) => {
        ...
        const operation = beginThreadOperation();
        try {
            await fetchAgentJson<AgentThreadResponse>(endpoint, token, `/agent/codex/threads/${encodeURIComponent(threadId)}/resume`, {
                method: "POST",
                headers: { "content-type": "application/json" },
