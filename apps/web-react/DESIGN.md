...
本文件记录迁移前 Vue 首页的视觉实现基线，供 React 页面回归对照使用。文中的
`.vue` 文件和旧 `src/` 路径属于历史引用，不代表当前运行源码位置。
...
- 用户端只面向桌面浏览器，主要设计视口为 `1440x900`，最低支持视口为 `1280x720`。
- 不为手机或平板新增断点、单列重排、移动导航、触控手势或专属交互。
- 低于 1280px 宽度不属于布局与视觉验收范围；历史移动端 CSS 不继续扩展。
- 首页、AI 助手和全部创作工作台均按同一桌面边界执行。页面调整必须保持业务逻辑、流式响应、任务状态和创作数据不变。
- 完整规则见 [桌面端 UI 支持策略](../../docs/DESKTOP_UI_POLICY.md)。
...
1. 首屏品牌、CTA、真实作品在 `1280x720` 和 `1440x900` 桌面视口可见。
