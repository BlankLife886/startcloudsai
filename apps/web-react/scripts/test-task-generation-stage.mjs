...
try {
  ...
  assert.ok(detailSource.includes('seconds={displayModule.seconds}'), '详情页画布生成中只显示当前版块的等待时间')
  assert.ok(detailSource.includes('secondsSince(item.state.startedAt, now)'), '详情页版块计时应以任务开始时间为起点，刷新后不归零')
