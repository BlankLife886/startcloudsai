...
import { QUEUE_CAPACITY_CODES, isInsufficientBalanceFailure } from "./submissionState.js";
...
export function generationButtonState({ authenticated = true, loading = false, referencesReady = true,
  ...
  const needsRecharge = remaining.length > 0 && remaining.every((entry) => isInsufficientBalanceFailure(entry.error));
  ...
  if (confirmation) {
    ...
    if (insufficient) {
      return {
        ...
        action: "recharge",
        disabled: false,
        points: points(confirmation.total),
        count: confirmation.count,
        title: "钱包余额不足，前往充值后再提交",
      };
    }
    return { ...result, state: "confirming", label: "等待确认", disabled: true, points: points(confirmation.total), count: confirmation.count, title: "请在费用弹窗中确认本次生成" };
  }
  ...
  if (needsRecharge) {
    return {
      ...
      label: "去充值",
      action: "recharge",
      title: `余额不足，还有 ${imageCount} 张待补交；充值后可继续提交`,
    };
  }
