...
export function promptNeedsRecentVisual(prompt) {
  ...
  const standaloneIteration = /^(再|继续|接着)?\s*(优化|美化|调整|修改|编辑|重做|改一下|调整一下)\s*(一下|看看|试试)?[。！!]?$/i
  if (standaloneIteration.test(text)) return true

  const visualIteration =
    /(?:(继续|接着|再|还要|也要)?\s*(优化|美化|调整|修改|编辑|重做|改一下|调整一下).{0,16}(布局|排版|界面|ui|弹窗|设计|样式|配色|间距|视觉|效果|细节|版本)|(布局|排版|界面|ui|弹窗|设计|样式|配色|间距|视觉|效果|细节|版本).{0,16}(继续|接着|再|还要|也要|优化|美化|调整|修改|编辑|重做|改一下|调整一下))/i
  if (visualIteration.test(text)) return true

  const feedbackIteration =
    /(?:(不太满意|不满意|不够满意|不是很满意|效果不好|效果不太好|不好看|不对劲).{0,20}(再|继续|重新|重来|换|更新|调整|优化|修改|改|迭代|出|来)|^(再|继续|重新|重来|换|更新|迭代|出|来).{0,8}(一|1|两|2)?(版|稿|张|次)[。！!]?$)/i
  if (feedbackIteration.test(text)) return true
...
export function resolveVisualContext(conversation, prompt, maxImages = 16, options = {}) {
  ...
  if (!options?.force && !promptNeedsRecentVisual(prompt)) return []
