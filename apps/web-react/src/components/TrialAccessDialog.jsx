  ...
  return (
    ...
    >
        ...
        <main className="trial-dialog__content" aria-live="polite" data-dialog-motion-item>
          ...
          {screen === "error" && <State icon="bi-cloud-slash" title={t("体验活动读取失败")} text={t(error)}><button type="button" className="is-primary" onClick={refreshApplication}>{t("重试")}</button></State>}
          {screen === "unavailable" && <State icon="bi-calendar2-x" title={t("本期申请已结束")} text={t("本期体验名额已满或活动已经关闭，暂时不能提交新的申请。")}><button type="button" className="is-secondary" onClick={onClose}>{t("关闭")}</button></State>}
          {screen === "auth" && <State icon="bi-person-plus" title={t("请先登录账号")} text={t("申请进度、审核结果和体验积分都会绑定到你的账号。")}><button type="button" className="is-primary" onClick={() => continueToAuth("register")}>{t("免费注册")}</button><button type="button" className="is-secondary" onClick={() => continueToAuth("login")}>{t("已有账号，去登录")}</button></State>}
          {screen === "pending" && <State icon="bi-hourglass-split" title={t("申请审核中")} text={t("申请已收到，审核结果会显示在这里并通过站内通知提醒你。")}><Summary application={application} features={features} t={t} locale={locale} /><button type="button" className="is-primary" onClick={refreshApplication}>{t("刷新状态")}</button><button type="button" className="is-secondary" onClick={onClose}>{t("我知道了")}</button></State>}
