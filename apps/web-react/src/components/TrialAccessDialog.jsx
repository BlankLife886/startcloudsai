...
export function TrialAccessDialog({ open, initialCampaign = null, onClose }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch((caught) => {
        if (caught?.name !== "AbortError") setError(caught?.message || "体验活动读取失败");
      })
      .finally(() => {
  ...
  function toggleOccupation(value) {
    setOccupations((current) => {
      ...
      if (current.length >= MAX_OCCUPATIONS) {
        notificationService.info("最多只能选择 4 个职业");
        return current;
      }
  ...
  function addCustomOccupation() {
    ...
    if (value.length < 2) {
      notificationService.info("请输入至少 2 个字的职业名称");
      return;
    }
  ...
  async function submit(event) {
    ...
    if (!occupations.length) {
      notificationService.info("请至少选择一个职业");
      return;
    }
    const normalizedReason = reason.trim();
    if (normalizedReason.length < 10) {
      notificationService.info("申请理由至少需要 10 个字");
      return;
    }
    ...
    try {
      ...
      notificationService.success("体验资格申请已提交");
    } catch (caught) {
      if (["trial_application_pending", "trial_application_approved"].includes(caught?.code)) {
        ...
        notificationService.error(caught?.message || "体验资格申请提交失败");
      }
    } finally {
      setSubmitting(false);
    }
  ...
  async function refreshApplication() {
    ...
    try {
      ...
      setError(caught?.message || "体验资格申请读取失败");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function claimReward() {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent("starclouds:wallet-updated", { detail: result }));
      setApplication(await getTrialAccessApplication());
      notificationService.success(`体验积分已到账 +${formatPoints(result?.grantCents || 0)}`);
    } catch (caught) {
      notificationService.error(caught?.message || "体验积分领取失败");
    } finally {
      setClaiming(false);
    }
  }
  ...
  return (
    ...
    >
        ...
        <aside className="trial-dialog__story" data-dialog-motion-item>
          ...
          <p className="trial-dialog__intro">告诉我们你的创作方式。审核通过后，可领取体验积分并进入获批工作台。</p>
          {campaign && (
            <section className="trial-dialog__campaign" aria-label="体验活动名额">
              <header><span>本期体验功能</span><strong>{campaign.full ? "名额已满" : `剩余 ${campaign.remaining ?? 0} 名`}</strong></header>
              <div className="trial-dialog__features">
                {sourceFeatures(campaign).map((feature) => <span key={feature.key}><i className={`bi ${feature.icon || "bi-stars"}`} />{feature.label}</span>)}
              </div>
              <div className="trial-dialog__track" aria-hidden="true"><i style={{ width: `${progress}%` }} /></div>
              <dl><div><dt>已申请</dt><dd>{applied}</dd></div><div><dt>总名额</dt><dd>{capacity || "—"}</dd></div><div><dt>活动截止</dt><dd>{formatDate(campaign.expiresAt)}</dd></div></dl>
            </section>
          )}
        ...
        <main className="trial-dialog__content" aria-live="polite" data-dialog-motion-item>
          ...
          {screen === "auth" && <State icon="bi-person-plus" title="请先登录账号" text="申请进度、审核结果和体验积分都会绑定到你的账号。"><button type="button" className="is-primary" onClick={() => continueToAuth("register")}>免费注册</button><button type="button" className="is-secondary" onClick={() => continueToAuth("login")}>已有账号，去登录</button></State>}
          {screen === "pending" && <State icon="bi-hourglass-split" title="申请审核中" text="申请已收到，审核结果会显示在这里并通过站内通知提醒你。"><Summary application={application} features={features} /><button type="button" className="is-primary" onClick={refreshApplication}>刷新状态</button><button type="button" className="is-secondary" onClick={onClose}>我知道了</button></State>}
          {screen === "approved" && <State icon="bi-patch-check-fill" title="体验资格已通过" text="真实功能权限已经生效，领取后积分可用于全部获批功能。"><Reward application={application} /><button type="button" className="is-primary is-wide" disabled={claiming} onClick={claimReward}>{claiming ? "领取中…" : "立即领取"}</button></State>}
          {screen === "expired" && <State icon="bi-clock-history" title="体验积分已过期" text="领取期限已经结束，请等待管理员重新发放体验积分。"><Reward application={application} /></State>}
          {screen === "redeemed" && <State icon="bi-check-lg" title="体验积分已到账" text="活动积分已经存入钱包，现在可以进入获批工作台开始体验。"><Reward application={application} /><div className="trial-dialog__launchers">{features.map((feature) => <button key={feature.key} type="button" onClick={() => { onClose?.(); navigate(feature.route || "/studio"); }}><i className={`bi ${feature.icon || "bi-stars"}`} /><span>{feature.label}</span><i className="bi bi-arrow-right" /></button>)}</div><button type="button" className="is-secondary" onClick={() => { onClose?.(); navigate("/wallet"); }}>查看钱包</button></State>}
          {screen === "apply" && (
            <form className="trial-dialog__form" onSubmit={submit}>
              {status === "rejected" && <div className="trial-dialog__rejected"><strong>上次申请未通过</strong><p>{application?.reviewNote || "可以更新资料后重新提交。"}</p></div>}
              <header><span className="trial-dialog__state-icon"><i className="bi bi-send-check" /></span><h3>申请体验资格</h3><p>选择你的职业，并简单说明准备如何使用这些功能。</p>{campaign?.nextPosition && <small>下一位申请者 #{campaign.nextPosition}</small>}</header>
              <fieldset><legend>你的职业 <em>{occupations.length}/{MAX_OCCUPATIONS}</em></legend><div className="trial-dialog__occupation-list">{OCCUPATIONS.map((item) => <button key={item} type="button" className={occupations.includes(item) ? "is-selected" : ""} onClick={() => toggleOccupation(item)}>{item}{occupations.includes(item) && <i className="bi bi-check-lg" />}</button>)}</div><div className="trial-dialog__custom"><input value={customOccupation} maxLength={50} placeholder="列表中没有？输入你的职业" onChange={(event) => setCustomOccupation(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); addCustomOccupation(); } }} /><button type="button" onClick={addCustomOccupation}>添加</button></div></fieldset>
              <label><span>申请理由</span><textarea value={reason} rows={5} minLength={10} maxLength={1000} required placeholder="请说明你想创作什么，以及准备如何使用平台…" onChange={(event) => setReason(event.target.value)} /><small>{reason.trim().length} / 1000</small></label>
              <button type="submit" className="is-primary is-wide" disabled={submitting}>{submitting ? "正在提交…" : "提交申请"}<i className="bi bi-arrow-right" /></button>
            </form>
          )}
        </main>
    </DialogMotion>
...
function Summary({ application, features }) {
  return <dl className="trial-dialog__summary"><div><dt>体验功能</dt><dd>{features.map((item) => item.label).join("、") || "—"}</dd></div><div><dt>你的职业</dt><dd>{application?.occupation || "—"}</dd></div><div><dt>申请理由</dt><dd>{application?.reason || "—"}</dd></div></dl>;
}

function Reward({ application }) {
  return <div className="trial-dialog__reward"><span>体验积分</span><strong>{formatPoints(application?.rewardCents || 0)}</strong><small>领取有效期至 {formatDate(application?.rewardExpiresAt)}</small></div>;
}
