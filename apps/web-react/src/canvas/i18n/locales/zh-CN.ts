export default {
    ...
    canvas: {
        ...
        node: {
            node: "节点",
            untitled: "未命名节点", renameHint: "双击修改节点名称", group: "组", nodeCount: "{{count}} 个节点", generating: "生成中", regenerating: "正在重新生成", regeneratingProgress: "正在重新生成 {{completed}}/{{total}}", failed: "生成失败", retry: "重试", missingPlugin: "缺少插件", missingPluginDescription: "节点类型“{{type}}”的插件未安装或未启用", generateImage: "用文本生图", generate: "生图", editText: "双击编辑文字", emptyImage: "空图片节点", emptyVideo: "空视频节点", emptyAudio: "空音频节点", audio: "音频", batchExpanded: "图片组已展开", batchCollapsed: "图片组已收起", createCopy: "创建副本", setPrimary: "设为主图",
        },
        sidePanel: {
            canvas: "画布", assets: "资产", prompts: "提示词库", resize: "调整左侧面板宽度", elements: "画布元素", select: "选择", searchNodes: "搜索节点", focusNode: "定位到节点", preview: "放大预览", workflowGroup: "工作流 {{index}} · {{name}}", standaloneNodes: "独立节点", noNodes: "画布暂无节点", noNodesHint: "在空白处双击，或从顶部工具栏添加节点", clearAll: "取消全选", selected: "已选 {{count}}", exporting: "正在导出选中元素…", exportName: "画布元素-{{count}}个", exported: "已导出 {{count}} 个元素", exportFailed: "导出失败，请重试",
    ...
    agent: {
        ...
        hosted: { heading: "AGENT", subtitle: "内置模型", placeholder: "描述你想改的画布，例如：在选中节点旁边加一个提示词", composerHint: "Enter 发送 · Shift+Enter 换行 · @ 引用节点", applying: "正在应用到画布…", toolReading: "正在读取画布…", toolModifying: "正在修改画布…", toolGenerating: "正在处理生成任务…", toolWorking: "正在执行{{tool}}…", applied: "已应用到画布", failed: "画布助手处理失败", authFailed: "对话模型认证失效，请在后台检查对话模型服务商的 API Key。", interrupted: "连接意外中断。已启动的生成任务仍会保留在画布中，请查看节点状态后重试。", emptyTitle: "从一句话改画布", empty: "描述布局、节点或生成流程，也可以点下面的示例开始。", suggestions: { layout: "整理当前画布的节点布局", flow: "根据选中节点创建生图流程", prompt: "在选中节点旁边加一个提示词", status: "查看当前生成任务状态" }, newChat: "新对话", linked: "已连线 {{count}} 条", appliedCount: "已执行 {{count}} 项操作", addedNodes: "新增 {{count}} 个节点", readCanvas: "已读取画布", generationStatus: "生成状态", generationTriggered: "已触发 {{count}} 个节点生成", generationSelectionSkipped: "跳过 {{count}} 个非有效图片节点", generationSucceeded: "成功 {{count}}", generationRunning: "进行中 {{count}}", generationFailed: "失败 {{count}}", generationIdle: "暂无生成任务", openedPage: "已打开 {{path}}", attachmentNodes: "已将 {{count}} 张图片放到画布", listedItems: "共 {{count}} 条", assetAdded: "已加入素材", reasoningStandard: "标准", reasoningExtended: "深度", reasoningStandardWithPrice: "标准 · {{price}}积分", reasoningExtendedWithPrice: "深度 · {{price}}积分", turnPrice: "{{price}} 积分/轮", turnPriceSale: "{{price}} 积分/轮", generationChargedSeparately: "Agent 按轮计费，模型生成费用另计", reasoningNotReturned: "上游模型消耗了推理 token，但服务商没有返回可展示的推理摘要。", noHistory: "这个画布还没有对话记录", deleteChat: "删除对话", justNow: "刚刚", minutesAgo: "{{count}} 分钟前", yesterday: "昨天" },
