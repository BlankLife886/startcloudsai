...
export function isSeedanceVideoConfig(config: AiConfig | Pick<AiConfig, "model" | "videoModel">) {
    return modelOptionName(config.model || config.videoModel).toLowerCase().includes("seedance");
}
