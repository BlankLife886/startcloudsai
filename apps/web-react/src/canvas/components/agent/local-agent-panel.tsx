    ...
    const runToolCall = async (endpoint: string, token: string, payload: AgentPendingToolCall) => {
        if (isSiteTool(payload.name)) {
            try {
                ...
                const result = await runSiteTool(payload.name, payload.input || {}, { canvasSnapshot: canvasContextRef.current?.snapshot || null });
            ...
            if (generated.length) {
                ...
                if (context) {
                    ...
                    const ops = generated.map<CanvasAgentOp>((image, index) => {
                        const size = cardSizeForMedia(image.upload.width, image.upload.height);
                        return {
                            type: "add_node",
                            id: `image-${createId()}`,
                            nodeType: "image",
...
async function attachmentNodeOps(endpoint: string, token: string, clientId: string, value: unknown): Promise<CanvasAgentOp[]> {
    ...
    return await Promise.all(
        nodes.map(async (value) => {
            ...
            const size = cardSizeForMedia(image.width, image.height);
