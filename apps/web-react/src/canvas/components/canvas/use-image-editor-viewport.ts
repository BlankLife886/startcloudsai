...
export function useImageEditorViewport(image: ImageSize | null, open: boolean) {
    ...
    useEffect(() => {
        ...
        const handleWheel = (event: WheelEvent) => {
            ...
            if (zoom > minZoom + 0.001) {
                viewportElement.scrollLeft += event.shiftKey ? event.deltaY : event.deltaX;
                viewportElement.scrollTop += event.shiftKey ? 0 : event.deltaY;
                return;
            }
            if (Math.abs(event.deltaY) > Math.abs(event.deltaX) && event.deltaY < 0) {
                setZoomAround(zoom * zoomStep, event.clientX, event.clientY);
            }
        };
        viewportElement.addEventListener("wheel", handleWheel, { capture: true, passive: false });
        return () => viewportElement.removeEventListener("wheel", handleWheel, { capture: true });
    ...
    return {
        ...
        isPanning,
        spacePressed,
        scrollClassName: `canvas-editor-viewport${zoom > minZoom + 0.001 ? " is-scrollable" : ""}`,
        panHandlers: {
