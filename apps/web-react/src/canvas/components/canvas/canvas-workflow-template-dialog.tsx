...
import { CANVAS_WORKFLOW_TEMPLATE_CATEGORIES, CANVAS_WORKFLOW_TEMPLATES, type CanvasWorkflowTemplate, type CanvasWorkflowTemplateCategory } from "@/templates/canvas-workflow-templates";

type TemplateDialogProps = {
    open: boolean;
    onClose: () => void;
    onUse: (template: CanvasWorkflowTemplate) => void;
};

export function CanvasWorkflowTemplateDialog({ open, onClose, onUse }: TemplateDialogProps) {
    const [category, setCategory] = useState<"all" | CanvasWorkflowTemplateCategory>("all");
    const [query, setQuery] = useState("");
    const filtered = useMemo(() => {
        const normalized = query.trim().toLowerCase();
        return CANVAS_WORKFLOW_TEMPLATES.filter((template) => {
        ...
        <Modal open={open} onCancel={onClose} footer={null} width={1180} centered destroyOnHidden title={null} className="canvas-template-modal" rootClassName="canvas-template-modal-root">
            <header className="canvas-template-modal__header">
                ...
                <div className="canvas-template-modal__metrics" aria-label="模板统计">
                    <span><strong>{CANVAS_WORKFLOW_TEMPLATES.length}</strong> 模板</span>
                    <span><strong>{new Set(CANVAS_WORKFLOW_TEMPLATES.map((item) => item.nodeCount)).size}</strong> 节点档位</span>
            ...
            <div className="canvas-template-toolbar">
                <div className="canvas-template-tabs" role="tablist" aria-label="模板分类">
                    <button type="button" className={category === "all" ? "is-active" : ""} onClick={() => setCategory("all")}>全部 <span>{CANVAS_WORKFLOW_TEMPLATES.length}</span></button>
                    {CANVAS_WORKFLOW_TEMPLATE_CATEGORIES.map((item) => (
                    ...
                    <article key={template.id} className="canvas-template-card" style={{ "--template-accent": template.accent } as CSSProperties}>
                        ...
                        <div className="canvas-template-card__body">
                            ...
                            <div className="canvas-template-card__footer">
                                ...
                                <button type="button" onClick={() => onUse(template)}>使用模板</button>
                            </div>
                        </div>
                    </article>
                ))}
            </div>
            {!filtered.length ? <div className="canvas-template-empty">没有匹配的模板</div> : null}
        </Modal>
    );
}
