...
import type { CanvasResourceIndex } from "@/lib/canvas/canvas-resource-index";
...
export function buildNodeResponseMessages(context: NodeGenerationContext): AiTextMessage[] {
    const prompt = buildImageReferencePromptText(context.prompt, context.referenceImages);
    ...
    return [
        {
            role: "user",
            content: [{ type: "text" as const, text: prompt }, ...context.referenceImages.map((image) => ({ type: "image_url" as const, image_url: { url: image.dataUrl } }))],
        },
    ];
}
