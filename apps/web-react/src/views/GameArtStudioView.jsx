...
function validImageCount(value) {
  ...
  return count >= 1 && count <= IMAGE_COUNT_HARD_MAX ? count : 0;
}
...
export function GameArtStudioView() {
  ...
  const [imageCount, setImageCount] = useState(validImageCount(settings.imageCount) || 1);
  // 张数跟随所选模型的「单次张数」配置。
  const countChoices = useMemo(() => imageCountChoices(currentModel || {}, imageCount), [currentModel, imageCount]);
  useEffect(() => {
    if (!currentModel) return;
    setImageCount((current) => clampImageCount(current, currentModel, 1));
  }, [currentModel]);
  ...
  const supportsReference = modelCapabilities.maxReferenceImages > 0;
  const hasReference = supportsReference && Boolean(inputFile || referenceUrl);
  const transparentEnabled = modelCapabilities.transparentBackground && currentState.toggles.transparent === true;
  const aspectOptions = useMemo(
    () => currentType.aspects.filter((ratio) => modelCapabilities.aspectRatios.includes(ratio)),
    [currentType, modelCapabilities.aspectRatios],
  );
  ...
  const downloadGameArt = useCallback(
    (url, filename) => runDownload(
      () => downloadAuthenticatedMedia(url, filename),
      { successMessage: "图片已开始下载" },
    ),
    [runDownload],
  );
  ...
  useEffect(() => {
    if (!currentModel) return;
    if (!aspectOptions.includes(currentState.aspect)) {
      patchState({ aspect: aspectOptions[0] || "" });
    }
    if (!modelCapabilities.transparentBackground && currentState.toggles.transparent) {
      patchToggle("transparent", false);
    }
  }, [aspectOptions, currentModel, currentState.aspect, currentState.toggles.transparent, modelCapabilities.transparentBackground]);
  ...
  async function generate() {
    ...
    const result = await jobs.generate({
      ...
      file: supportsReference ? inputFile : null,
      sourceUrl: supportsReference ? referenceUrl : "",
      referencePreviewUrl: supportsReference ? referenceUrl || sourcePreview : "",
      aspectRatio: currentState.aspect,
      count: imageCount,
      quality: modelCapabilities.qualities.length ? (hdMode ? "high" : "medium") : "",
  ...
  return <main ref={rootRef} className={`game-art-studio${isDark ? "" : " is-light"}`} style={backgroundStyle}>
    ...
    <section className="ga-main"><div className="ga-workspace"><section className="ga-canvas" data-studio-enter>
      ...
      <div className={`ga-composer${dropActive ? " is-drop" : ""}`} data-studio-enter onDragOver={(event) => { event.preventDefault(); setDropActive(true); }} onDragLeave={() => setDropActive(false)} onDrop={(event) => { event.preventDefault(); setDropActive(false); applyFile(event.dataTransfer.files?.[0]); }}>
        {supportsReference && <div className={`ga-composer-ref${referenceUrl || sourcePreview ? " has-image" : ""}`}><button type="button" className="ga-composer-ref-pick" title="参考图：拖入 / 粘贴 / 点击上传" onClick={() => fileInputRef.current?.click()}>{referenceUrl ? <AuthenticatedImage src={referenceUrl} alt="参考图" maxDimension={160} /> : sourcePreview ? <img src={sourcePreview} alt="参考图" /> : <><i className="bi bi-image" /><span>参考图</span></>}</button>{(inputFile || referenceUrl) && <button type="button" className="ga-composer-ref-clear" title="移除参考图" onClick={() => { setInputFile(null); setSourcePreview(""); setReferenceUrl(""); }}><i className="bi bi-x-lg" /></button>}</div>}
        ...
        <div className="ga-composer-features"><div className="ga-feature-grid" role="group" aria-label="生成扩展设置"><button type="button" className={`ga-feature-trigger${composerPanel === "quality" ? " is-open" : ""}${modelCapabilities.qualities.length && hdMode ? " is-on" : ""}`} onClick={() => setComposerPanel(composerPanel === "quality" ? "" : "quality")}><i className="bi bi-bounding-box-circles" /><span><strong>清晰度</strong><small>{currentClarity.label}{modelCapabilities.qualities.length ? ` · ${hdMode ? "HD" : "标准"}` : ""}</small></span><i className="bi bi-chevron-up" /></button>{modelCapabilities.transparentBackground && <button type="button" className={`ga-feature-trigger${transparentEnabled ? " is-on" : ""}`} role="switch" aria-checked={transparentEnabled} onClick={() => patchToggle("transparent", !transparentEnabled)}><i className="bi bi-transparency" /><span><strong>移除背景</strong><small>{currentModel?.label || "当前模型"}</small></span><span className="ga-feature-switch"><span /></span></button>}{supportsReference && <button type="button" disabled={!hasReference} className={`ga-feature-trigger${composerPanel === "reference" ? " is-open" : ""}${hasReference ? " is-on" : ""}`} onClick={() => hasReference && setComposerPanel(composerPanel === "reference" ? "" : "reference")}><i className="bi bi-image" /><span><strong>参考图约束</strong><small>{hasReference ? currentReferenceConstraint.label : "未添加"}</small></span><i className="bi bi-chevron-up" /></button>}<button type="button" className={`ga-feature-trigger${composerPanel === "output" ? " is-open" : ""}`} onClick={() => setComposerPanel(composerPanel === "output" ? "" : "output")}><i className="bi bi-aspect-ratio" /><span><strong>{aspectOptions.length ? "比例与数量" : "生成数量"}</strong><small>{aspectOptions.length ? `${currentState.aspect} · ` : ""}{imageCount} 张</small></span><i className="bi bi-chevron-up" /></button></div>
          {composerPanel && <section className={`ga-feature-popover is-${composerPanel}`}><header><strong>{composerPanel === "quality" ? "清晰度与高清生产" : composerPanel === "reference" ? "参考图约束" : aspectOptions.length ? "比例与数量" : "生成数量"}</strong><button type="button" title="关闭" onClick={() => setComposerPanel("")}><i className="bi bi-x-lg" /></button></header>{composerPanel === "quality" ? <><div className="ga-feature-segment">{CLARITY_OPTIONS.map((item) => <button key={item.id} type="button" className={clarity === item.id ? "is-on" : ""} onClick={() => setClarity(item.id)}>{item.label}</button>)}</div>{modelCapabilities.qualities.length > 0 && <button className={`ga-feature-option${hdMode ? " is-on" : ""}`} type="button" onClick={() => setHdMode((value) => !value)}><span><i className="bi bi-badge-hd" /><strong>高清生产模式</strong></span><span className="ga-feature-switch"><span /></span></button>}</> : composerPanel === "reference" ? <div className="ga-reference-options">{REFERENCE_CONSTRAINT_OPTIONS.map((item) => <button key={item.id} type="button" className={currentState.referenceConstraint === item.id ? "is-on" : ""} onClick={() => patchState({ referenceConstraint: item.id })}><strong>{item.label}</strong><small>{item.prompt}</small></button>)}</div> : <div className="ga-output-settings">{aspectOptions.length > 0 && <div className="ga-output-setting"><strong>输出比例</strong><div className="ga-output-options">{aspectOptions.map((ratio) => <button key={ratio} type="button" className={currentState.aspect === ratio ? "is-on" : ""} onClick={() => patchState({ aspect: ratio })}>{ratio}</button>)}</div></div>}<div className="ga-output-setting"><strong>生成数量</strong><div className="ga-output-options is-count">{countChoices.map((count) => <button key={count} type="button" className={imageCount === count ? "is-on" : ""} onClick={() => setImageCount(count)}>{count} 张</button>)}</div></div></div>}</section>}
        </div>
      ...
      </div>{supportsReference && <input ref={fileInputRef} hidden type="file" accept="image/*" onChange={(event) => { applyFile(event.target.files?.[0]); event.target.value = ""; }} />}
    </section>
