...
export function GameArtStudioView() {
  ...
  return <main ref={rootRef} className={`game-art-studio${isDark ? "" : " is-light"}`} style={backgroundStyle}>
    ...
    {fullscreenOpen && jobs.activeOutput && <EcommerceFullscreenPreview sourceUrl={jobs.activeOutput} displaySourceUrl={activeEntry?.displayUrl || ""} title={currentTypeHeading} gallery={typeEntries.map((entry) => entry.url)} onSelect={jobs.setActiveOutput} onClose={() => setFullscreenOpen(false)} onDownload={() => downloadAuthenticatedMedia(jobs.activeOutput, `game-${assetType}.png`)} />}
