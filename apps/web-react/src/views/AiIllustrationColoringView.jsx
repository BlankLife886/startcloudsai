...
function MediaImage({ src, fallbackSrc = "", alt = "", className = "", ...props }) {
  ...
  return <AuthenticatedImage {...props} className={className} src={value} alt={alt} fallbackSrc={fallbackSrc} />;
}
...
export function AiIllustrationColoringView() {
  ...
  // 展示图：大图舞台预览用；下载/二次染色仍走 resultUrl 原图。
  const resultDisplayUrl = active?.resultDisplayUrl || "";
  const downloadTitle = active?.title || "插画染色";
  const downloadColoringResult = useCallback(
    () => runDownload(
      () => downloadAuthenticatedMedia(resultUrl, `${downloadTitle}.png`),
      { successMessage: "染色结果已开始下载" },
    ),
    [downloadTitle, resultUrl, runDownload],
  );
