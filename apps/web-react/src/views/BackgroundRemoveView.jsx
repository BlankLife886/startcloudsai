...
export function BackgroundRemoveView() {
  ...
  const downloadOriginal = useCallback(
    () => runDownload(
      () => downloadAuthenticatedMedia(resultUrl, `background-removed-${Date.now()}.png`),
      { successMessage: "原图已开始下载" },
    ),
    [resultUrl, runDownload],
  );
  ...
  const requestRemoval = useCallback(async () => {
    if (requestAuth({ featureLabel: "背景移除" })) return;
    if (!canRun) return;
  ...
  }, [auth.user?.requireCostConfirm, canRun, executeRemoval, requestAuth, unitPrice]);
      ...
      <header className="br-header">
        <div className="br-header__copy">
          ...
          <div className="br-meta">
            ...
            <span className="br-chip"><DownloadIcon />本页压缩下载</span>
