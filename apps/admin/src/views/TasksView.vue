...
const statusCards = computed(() => [
  { value: '', label: '全部任务', count: summary.value.total, tone: 'all', hint: `今日新增 ${summary.value.today}` },
  { value: 'queued', label: '等待中', count: summary.value.queued, tone: 'queued', hint: '尚未开始执行' },
  { value: 'running', label: '运行中', count: summary.value.running, tone: 'running', hint: '正在占用执行槽位' },
  { value: 'failed', label: '需要处理', count: summary.value.failed, tone: 'failed', hint: '可查看错误并重试' },
  { value: 'succeeded', label: '已成功', count: summary.value.succeeded, tone: 'succeeded', hint: '已完成交付' },
  { value: 'canceled', label: '已取消', count: summary.value.canceled, tone: 'canceled', hint: '主动取消或终止' },
])

const activeFilterCount = computed(
  () => [filters.type, filters.status, filters.user.trim(), filters.errorCode.trim()].filter(Boolean).length,
)
...
function taskPrompt(task: AdminTask) {
  return task.prompt?.trim() || (task.source === 'assistant' ? 'AI 助手任务' : '未填写提示词')
}
...
const detailActiveMediaUrl = computed(() => detailMediaUrls.value[detailMediaIndex.value] ?? '')
...
async function cancel(task: AdminTask) {
  ...
  try {
    ...
    ElMessage.success(task.source === 'assistant' ? 'AI 助手任务已取消' : '已取消并解冻费用')
    detailVisible.value = false
    refresh()
  } finally {
    acting.value = false
...
<template>
  <div class="task-monitor">
    <section class="task-filter-panel">
      ...
      <el-select v-model="filters.type" placeholder="全部任务类型" clearable @change="reset">
      ...
      <el-button :disabled="!activeFilterCount" @click="clearFilters">清除条件</el-button>
    </section>
    ...
    <section class="status-overview" aria-label="任务状态汇总">
      <button
        ...
        type="button"
        class="status-card"
        :class="[`is-${card.tone}`, { 'is-active': filters.status === card.value }]"
        @click="applyStatus(card.value)"
      >
        <span class="status-card__dot" />
        <span class="status-card__copy"><small>{{ card.label }}</small><strong>{{ card.count }}</strong><em>{{ card.hint }}</em></span>
      </button>
    </section>
    ...
    <section class="task-list-panel">
      <header class="task-list-panel__head">
        <div>
          <strong>{{ filters.status ? TASK_STATUS_LABELS[filters.status] : '全部任务' }}</strong>
          <span>当前页 {{ items.length }} 条<span v-if="activeFilterCount"> · {{ activeFilterCount }} 个筛选条件</span></span>
        </div>
      ...
      <div class="task-table-shell">
        ...
        >
            ...
            <template #default="{ row }">
              <div class="task-primary" @click="openDetail(row as AdminTask)">
                <div class="task-primary__id">
                  <span class="mono" :title="row.id">{{ compactTaskId(row.id) }}</span>
                  <button type="button" title="复制任务 ID" @click.stop="copyTaskId(row.id)">
          ...
          <el-table-column label="任务内容" width="240">
            <template #default="{ row }">
              <div class="task-prompt-cell">
                <span :title="taskPrompt(row as AdminTask)">{{ taskPrompt(row as AdminTask) }}</span>
                <button type="button" title="复制任务内容" @click.stop="copyTaskPrompt(row as AdminTask)">
                  <el-icon><CopyDocument /></el-icon>
                </button>
              </div>
            </template>
          ...
          <el-table-column label="任务类型" width="118" align="center">
            <template #default="{ row }">
              <div class="task-type"><strong>{{ taskTypeLabel(row.type) }}</strong></div>
            </template>
          </el-table-column>
          <el-table-column label="任务来源" width="104" align="center">
            <template #default="{ row }">
              <span class="task-source" :class="{ 'is-assistant': row.source === 'assistant' }">{{ taskSourceLabel(row as AdminTask) }}</span>
            </template>
          </el-table-column>
          <el-table-column label="用户" min-width="175">
            <template #default="{ row }">
              <span class="task-user" :title="taskUser(row as AdminTask)">{{ taskUser(row as AdminTask) }}</span>
            </template>
          </el-table-column>
          <el-table-column label="结果" width="82" align="center">
            <template #default="{ row }">
              <div class="task-numbers"><strong>{{ taskOutputCount(row as AdminTask) }} / {{ taskCount(row as AdminTask) }}</strong></div>
            </template>
          </el-table-column>
          <el-table-column label="费用" width="100" align="center">
            <template #default="{ row }">
              <strong class="task-cost">¥ {{ fenToYuan(row.costCents) }}</strong>
            </template>
          </el-table-column>
          <el-table-column label="耗时" width="110" align="center">
            <template #default="{ row }">
              <div class="task-time"><strong>{{ taskDuration(row as AdminTask) }}</strong></div>
            </template>
          </el-table-column>
          <el-table-column label="异常" min-width="180">
            <template #default="{ row }">
              <div v-if="row.errorCode" class="task-error" :title="row.errorMessage ?? ''"><strong>{{ row.errorCode }}</strong><small>{{ row.errorMessage || '点击详情查看错误信息' }}</small></div>
    ...
    <el-drawer v-model="detailVisible" :with-header="false" size="640px" class="task-detail-drawer">
      <template v-if="detail">
        <header class="detail-hero">
          <div>
            <span>任务详情</span>
            <h2>{{ taskTypeLabel(detail.type) }}</h2>
            <button type="button" @click="copyTaskId(detail.id)"><span class="mono">{{ detail.id }}</span><el-icon><CopyDocument /></el-icon></button>
          </div>
          <span class="task-status is-large" :class="`is-${detail.status}`"><i />{{ TASK_STATUS_LABELS[detail.status] ?? detail.status }}</span>
        </header>

        <section v-if="detailOutputUrls.length || detailInputUrls.length" class="detail-media-panel">
          ...
          <div class="detail-media-stage">
            <el-image
              ...
              hide-on-click-modal
            />
            <span class="detail-media-counter">{{ detailMediaIndex + 1 }} / {{ detailMediaUrls.length }}</span>
          </div>
          <div v-if="detailMediaUrls.length > 1" class="detail-media-thumbs">
            ...
            >
              <img :src="url" :alt="`${detailMediaMode === 'output' ? '生成结果' : '输入参考'} ${index + 1}`" loading="lazy" />
            </button>
          </div>
        </section>
        <div v-else class="detail-media-empty">
          <span>此任务没有图片产物</span>
          <small>{{ detail.status === 'failed' ? '请查看下方异常信息' : '对话任务或产物仍在处理中' }}</small>
        </div>
        ...
        <el-descriptions :column="2" size="small" border class="detail-meta">
          ...
          <el-descriptions-item label="耗时">{{ taskDuration(detail) }}</el-descriptions-item>
...
.refresh-state small { color: var(--ink-3); }
...
.status-card:hover { background: var(--surface-2); }
.status-card.is-active { color: var(--accent-ink); background: var(--accent-soft); }
...
.status-card.is-running { color: var(--info); }
.status-card.is-failed { color: var(--danger); }
.status-card.is-succeeded { color: var(--success); }
.status-card.is-queued { color: var(--warning); }
...
.status-card__copy small { color: var(--ink-3); font-size: 10px; }
.status-card__copy strong { color: var(--ink); font-size: 19px; line-height: 1.15; }
...
.task-filter-panel {
  display: grid;
  grid-template-columns: auto minmax(240px, 1.2fr) minmax(150px, 0.5fr) minmax(170px, 0.6fr) auto auto;
  align-items: center;
  gap: 8px;
  padding: 10px;
  border: 1px solid var(--border);
...
.task-list-panel__head > div { display: flex; align-items: baseline; gap: 8px; }
.task-list-panel__head strong { color: var(--ink); font-size: 14px; }
.task-list-panel__head span { color: var(--ink-3); font-size: 10px; }
...
.task-list-panel__footer > span { color: var(--ink-3); font-size: 10px; }
.task-list-panel__footer :deep(.pager-bar) { margin: 0; }

.task-primary { min-width: 0; cursor: pointer; }
.task-primary__id { display: flex; align-items: center; gap: 5px; color: var(--ink-2); }
...
.task-prompt-cell:hover button { opacity: 1; }
.task-primary__id button:hover,
.task-prompt-cell button:hover { color: var(--accent-ink); background: var(--surface-3); }
...
.task-image-empty { display: block; width: 40px; margin: 0 auto; color: var(--ink-3); text-align: center; }
...
.task-status i { width: 7px; height: 7px; border-radius: 50%; background: currentColor; }
.task-status.is-running { color: var(--info); }
.task-status.is-queued { color: var(--warning); }
.task-status.is-failed { color: var(--danger); }
.task-status.is-succeeded { color: var(--success); }
.task-status.is-large { padding: 6px 9px; border-radius: 999px; background: var(--surface); }
...
.task-numbers strong,
.task-time strong,
.task-error strong { overflow: hidden; color: var(--ink-2); font-size: 11px; text-overflow: ellipsis; white-space: nowrap; }
.task-type small,
.task-numbers small,
.task-time small,
.task-error small { overflow: hidden; color: var(--ink-3); font-size: 9px; text-overflow: ellipsis; white-space: nowrap; }
.task-error strong { color: var(--danger); font-family: ui-monospace, SFMono-Regular, Menlo, monospace; }
.task-error-empty { color: var(--ink-3); }
.task-user { display: block; overflow: hidden; color: var(--ink-2); font-size: 11px; text-overflow: ellipsis; white-space: nowrap; }
.task-source {
  display: inline-flex;
  padding: 4px 7px;
  border-radius: 6px;
...
.task-source.is-assistant { color: var(--accent-ink); background: var(--accent-soft); }
:deep(.el-table) { --el-table-row-hover-bg-color: var(--surface-2); height: 100% !important; }
:deep(.el-table th.el-table__cell) { height: 38px; color: var(--ink-3); background: var(--surface-2); font-size: 10px; font-weight: 600; }
:deep(.el-table td.el-table__cell) { height: 52px; padding: 5px 0; }
:deep(.el-table .task-row.is-failed td.el-table__cell) { background: color-mix(in srgb, var(--danger-soft) 28%, var(--surface)); }
:deep(.el-table .task-row.is-running td.el-table__cell) { background: color-mix(in srgb, var(--info-soft) 22%, var(--surface)); }
...
.detail-hero > div { display: grid; min-width: 0; gap: 2px; }
.detail-hero > div > span { color: var(--ink-3); font-size: 10px; letter-spacing: 0.08em; }
.detail-hero h2 { margin: 0; font-size: 20px; }
.detail-hero button { display: flex; min-width: 0; align-items: center; gap: 6px; padding: 0; border: 0; color: var(--ink-3); background: transparent; cursor: pointer; }
.detail-hero button .mono { overflow: hidden; max-width: 360px; text-overflow: ellipsis; }
.detail-media-panel {
  overflow: hidden;
  margin-bottom: 14px;
  border: 1px solid var(--border);
...
.detail-media-tabs button:hover { color: var(--ink); background: var(--surface-2); }
.detail-media-tabs button.is-active { color: var(--accent-ink); background: var(--accent-soft); }
...
.detail-media-stage {
  ...
  overflow: hidden;
  background:
    radial-gradient(circle at 50% 24%, color-mix(in srgb, var(--accent) 10%, transparent), transparent 44%),
    var(--surface-3);
}
...
.detail-media-stage::before {
  ...
  inset: 0;
  background-image:
    linear-gradient(45deg, color-mix(in srgb, var(--ink) 3%, transparent) 25%, transparent 25%),
    linear-gradient(-45deg, color-mix(in srgb, var(--ink) 3%, transparent) 25%, transparent 25%),
    linear-gradient(45deg, transparent 75%, color-mix(in srgb, var(--ink) 3%, transparent) 75%),
    linear-gradient(-45deg, transparent 75%, color-mix(in srgb, var(--ink) 3%, transparent) 75%);
...
.detail-media-thumbs button:hover { border-color: var(--ink-3); transform: translateY(-1px); }
.detail-media-thumbs button.is-active {
  border-color: var(--accent);
  box-shadow: 0 0 0 2px var(--accent-soft);
}
.detail-media-thumbs img { display: block; width: 100%; height: 100%; border-radius: 6px; object-fit: cover; }
...
.detail-media-empty span { font-size: 12px; font-weight: 650; }
.detail-media-empty small { color: var(--ink-3); font-size: 10px; }
...
@media (max-width: 1180px) {
  ...
  .status-card:nth-child(-n + 3) { border-bottom: 1px solid var(--border); }
  .task-filter-panel { grid-template-columns: auto minmax(220px, 1fr) minmax(150px, 0.5fr) minmax(160px, 0.6fr) auto; }
  .task-filter-panel > .el-button:last-child { display: none; }
}

@media (max-width: 760px) {
  .task-monitor { height: auto; min-height: 100%; grid-template-rows: auto; padding: 12px; overflow: auto; }
  .task-filter-panel__refresh {
    width: 100%;
    grid-column: 1 / -1;
    padding: 0 0 8px;
  ...
  .refresh-state { margin-right: auto; }
  .status-overview { grid-template-columns: repeat(2, minmax(0, 1fr)); }
  .status-card { border-bottom: 1px solid var(--border); }
  .status-card:nth-child(2n) { border-right: 0; }
  .task-filter-panel { grid-template-columns: 1fr 1fr; }
  .task-filter-panel__lead { grid-column: 1 / -1; }
  .task-list-panel { min-height: 560px; }
  .task-list-panel__footer > span { display: none; }
  .detail-media-panel__head > small { display: none; }
  .detail-media-stage { height: min(52vh, 380px); }
}

@media (prefers-reduced-motion: reduce) {
  .refresh-state > span.is-live { animation: none; }
}
</style>
