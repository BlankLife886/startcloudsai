...
- 用户端仅支持 Gmail、Googlemail、QQ 邮箱验证码认证。`/auth/email/verify` 会在同一事务内验证验证码，并为首次邮箱自动创建用户、钱包、初始积分和 session；已有用户直接创建 session。Gmail 点号、加号标签和 Googlemail 地址统一规范化。不提供用户密码或第三方 OAuth 登录。用户 Cookie 为 `sc_session`，有效期 30 天。
