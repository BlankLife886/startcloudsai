...
func TestOSSUploadDoesNotUseUnsupportedAWSChunkedTrailer(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if got := r.Header.Get("Cache-Control"); got != "private, max-age=31536000, immutable" {
...
func TestObjectUploadCacheControlDistinguishesVersionedKeys(t *testing.T) {
	tests := []struct {
		...
		{key: "tasks/user/task/original/result.png", want: "private, max-age=31536000, immutable"},
		{key: "prompt-covers/item/version.png", want: "private, max-age=31536000, immutable"},
		{key: "canvas-template-covers/item/version.webp", want: "private, max-age=31536000, immutable"},
		{key: "ecommerce-catalog/item/version.jpg", want: "private, max-age=31536000, immutable"},
