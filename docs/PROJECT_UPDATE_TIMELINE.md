...
本文记录 StarCloudsAI 的代码演进。2026-09-22 补充了下方当前快照；第 1—20 节保留原 2026-09-01 整理的历史内容，其中“当前”“未提交”、测试通过和部署结论均指当时，不作为今天的事实。现行行为以 [文档索引](README.md) 中的专项文档为准。

## 2026-09-22 当前工作区快照

| 项目 | 本次只读核对 |
| --- | --- |
| HEAD | `3ebe980fa2c87bdef424ac434537090c13097bb6`，提交日期 2026-09-18 |
| 分支 | `codex/publish-current-project` |
| HEAD 可达提交数 | 162 |
| 源码最高迁移 | `00155_developer_api_profitability.sql`；不代表数据库已执行 |
| 工作区 | 存在跨前端、服务端及迁移的未提交修改；不固定持续变化的 diff 行数 |
| 验证/部署 | 本轮只同步文档并做静态检查，未运行业务回归、迁移或发布 |

2026-09-01 之后的已提交阶段：

- 9 月 1 日：平台运营/助手工具、媒体加载、支付快照及订单恢复；增加不依赖服务器 Git 的集成发布。
- 9 月 5 日：助手、App 与后台工作流、匹配 Worker 版本发布及用户更新记录。
- 9 月 9 日：账务/执行更新及维护窗口部署、Worker 实际就绪检查、开放 API 文档打包。
- 9 月 13 日：画布、后台、App 和生成体验改进。
- 9 月 17 日：内联分镜流程、Responses API 与图片技能 OAuth、真实图片技能库、可恢复画布批次及幂等键修正。
- 9 月 18 日：社区画廊渐进加载、Agent 工具步骤时间线。

当前尚未提交的实现进一步调整电商 Listing/A+/详情页工作台、技能本地/云端存储与 `@` 调用、助手方案自动授权及预算、Agent 准入、流式工具步骤、图片下载和开发者 Images 直通。对应新增迁移为 `00152`—`00155`：自动批准预算、技能 slug、移除旧装载表、开发者 API 利润流水。源码变化不等于这些能力已进入生产。

从旧版本升级时，`00154` 会删除 `user_skill_bindings`，不能未经兼容检查就混跑旧服务与候选 API。当前升级说明见 [维护发布](MAINTENANCE_RELEASE.md)。

## 1. 历史时间与版本边界（2026-09-01 快照）
...
后续发布应记录确切提交、实际数据库迁移历史及本轮验证结果；不沿用原 286 项 diff、迁移 117 或旧测试通过作为门禁。网站打包只归档已提交的网站代码并排除 App，不能把本地业务数据与密钥带入生产。版本相关的功能开关和部署脚本行为在专项文档中核对，不用历史“默认关闭”推断某个环境的现状。

本文是演进记录；现行接口、计费、技能和部署以 [完整文档索引](README.md) 链接的当前规范为准。
