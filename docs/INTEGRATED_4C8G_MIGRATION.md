...
```text
Browser -> HTTPS Nginx -> StartCloud gateway
                                  |-> Web / Admin / Go API
...
OSS bucket 保持私有。普通图片请求始终先访问稳定的 `/api/v1/files/{key}`，由 Go 服务
完成属主或公开权限校验，再通过香港内网 endpoint 从 OSS 流式返回。下载、Range、ETag
和缓存策略继续由站内文件接口处理；需要把图片交给外部上游时，使用公网 endpoint 生成
短期 OSS 预签名 URL。上线前必须验证无权限用户仍为 `401/404`，属主查看和下载不发生
跨域重定向。
