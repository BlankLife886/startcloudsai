...
管理端只面向桌面浏览器，最低支持视口为 `1280x720`，不新增或验收手机/平板适配。完整边界见 [桌面端 UI 支持策略](DESKTOP_UI_POLICY.md)。
...
- 筛选项按高频顺序排列；在受支持的桌面工作区内，搜索提交、重置和主要动作可以按内容宽度换行。
...
- 状态不仅依靠颜色，须同时有文字或图标。
- 表单和按钮按桌面鼠标与键盘操作设计；文字不得与徽标、图标重叠。
...
5. `1280x720` 和 `1440x900` 桌面布局无重叠、截断或不可达操作；不验收移动端。
6. 破坏性写操作有确认，并会被后端审计。
