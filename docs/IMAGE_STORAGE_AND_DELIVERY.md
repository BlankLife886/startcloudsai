...
1. 站内用户上传和 AI 生成的持久图片不存放在 API/Worker 容器项目目录中；标准图片直通不建立本站持久对象。
2. 站内最终图片存放在 `OBJECT_STORAGE_BUCKET` 指定的 S3 兼容私有对象存储；可使用 OSS、R2 或其他兼容服务。
3. 本地集成模板使用 MinIO，示例 Bucket 为 `startcloudsai-local`，数据在 Docker Volume 中；实际名称需核对配置。
...
适用于文生图、插画染色、UI 设计、AI 电商、模型图、游戏美术、背景移除、媒体工具和无限画布发起的普通图片任务。拼图仅保留历史任务数据，当前拼图由浏览器本地执行，不创建新云端任务。
