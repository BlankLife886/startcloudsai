...
2026-09-22 核对：当前客户端仍有 51 个 key。电商主菜单控制 12 个业务入口，旧 `clone` 模式没有独立的 `ecommerce.clone` key，不能假设所有 `tool` 参数都有一一对应的页面控制。技能库 key 对应真实本地/云端/官方技能页面，详见 [技能库现状](SKILL_PAGE_PLAN.md)。注册路由、菜单曝光和页面状态是不同层级，不应仅靠入口是否显示判断后端能力是否可用。
