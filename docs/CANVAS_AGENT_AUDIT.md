...
2026-09-22 现状补充：本文主体为 2026-09-06 的历史审查与当时测试结果，行号和缺陷状态不能直接作为当前版本结论。现码已加入工具投递 `executing/completed` 持久日志、未确认执行状态阻止自动重放、基于 requestId 的生成任务盐值、工作流 `inputSignature`/产物指纹校验，以及桥接工作流 `attemptId` 身份。对应实现为 `canvas-agent-tool-delivery.ts`、`canvas-task-api.ts`、`canvas-workflow.ts` 和 `use-agent-bridge.ts`。因此下文“只有内存结果缓存”“仅比较节点 ID”“用 startedAt 判断尝试”的缺陷描述属于修复前记录。

分镜后续进度见 [2026-09-15 审查与方案](CANVAS_STORYBOARD_AUDIT_AND_PLAN_2026-09-15.md)。本次文档核对没有重做付费模型、崩溃恢复或浏览器端到端验收，也不把代码中已有保护等同于全部故障场景已验证。
