...
- 普通助手的工作区操作按工具确认卡执行；图片方案可按用户的自动批准偏好自动执行符合条件的方案。画布有独立的工具确认策略，高风险操作仍需确认。不能把所有模式概括为每一步都必须手动确认；费用与权限以服务端规则为准。
...
技能库 `/skills` 支持本地、云端与官方文本技能，通过文生图、电商、助手和画布中的 `@` 提及使用；它不增加额外工具权限，保存技能也不会执行收费任务。详见 [技能库现状](SKILL_PAGE_PLAN.md)。
...
独立入口还包括 `/psd-decompose`（图片转分层 PSD）、`/holo-card`（闪光卡）和 `/holo-card/sample`（样卡）。素材当前使用 `/assets`，旧 `/materials` 重定向到该页面。业务工作台的功能是否开放还受页面控制、模型配置和账号权限影响；媒体工具中的视频/音频不能等同于无限画布已开放相应节点生成。
