...
核对日期：2026-09-22。当前仍使用公告快照、`pushId`/`pushedAt`、Redis 通知和公开 SSE；下列命令为回归入口，不是本次文档更新已经通过的测试记录。
...
Go HTTP 用例需要 `TEST_DATABASE_URL` 指向可创建临时库的隔离 PostgreSQL。后台 Playwright 用例不会自动启动管理端：先在另一个终端运行 `npm --prefix apps/admin run dev`，并安装对应 Playwright 浏览器；未传 `ADMIN_BASE_URL` 时该后台用例会跳过。主站 Vite 由 Playwright 配置启动，两组浏览器用例使用模拟 API，不推送真实公告。
