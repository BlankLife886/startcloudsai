...
核对日期：2026-09-22。下文“旧服务器没有 ChatGPT2API、新服务器保留既有 ChatGPT2API”的描述是这份迁移方案的场景前提，不是本次读取源码确认的线上事实。不同环境必须先按预检报告重建数据清单，本文不授予恢复或覆盖生产数据的操作权限。
...
开始迁移时先执行第 1、2 阶段。预检和图片来源清单没有核对完成前，不执行数据库恢复。本次文档同步没有连接服务器或执行上述任何阶段。
...
- 已核对当前 `00152`—`00155` 等待应用迁移，特别是 `00154` 删除旧技能装载表及新旧 API/Worker 的兼容性；不以最大编号代替实际迁移文件和 `goose_db_version` 历史；
