...
支持 `/api/image-tasks` 异步接口的 OpenAI 兼容服务商在提交后立即释放 Worker。同一服务商共享一个聚合轮询器，每次状态请求携带最多 100 个任务 ID；未完成则以 2-4 秒抖动重新计划。CRUN 创建 job 后也立即释放 Worker，由服务商级轮询器集中执行单轮 TaskInfo 查询。旧服务商返回 404/405 时自动回退同步 OpenAI Images 接口；这类线路仍会在生成期间占用 Worker，因此不应承担大规模在途容量。

### 多服务商容量加权

同一个公开模型可配置多条执行线路。后台为每个模型显式指定 `executionPoolId`；只有资源池相同且模型类型相同的线路才会互相分流，备用模型记录本身可以不公开。不同服务商可使用不同 adapter 或不同 `upstreamModel` 别名，不再依赖字符串自动猜测。未配置该字段的历史模型会按旧规则生成兼容资源池，后台下次保存时固化为显式配置。领取任务时在 PostgreSQL 全局执行锁内读取各服务商 running 数，排除达到 `maxConcurrency` 的线路，并选择 `running / maxConcurrency` 最低者。

网络错误、429、可重试 5xx 或服务商被停用时，任务保持原业务 ID、冻结金额和上游幂等信息重新入队。下一次领取优先排除刚失败的服务商并选择等价备用线路；只有池中没有其他可用线路时才回到原线路继续有限次数恢复。

例如服务商 A 容量 200、B 容量 600，并把两个模型线路指定到同一个调度资源池，800 个合成在途任务会分配为 200/600；第 801 个保持排队，直到任一线路释放容量。增加服务商只需要添加模型线路、选择已有资源池并设置容量，无需修改用户端。
