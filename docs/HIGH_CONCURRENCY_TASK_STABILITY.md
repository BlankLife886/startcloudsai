...
核对基线：2026-09-22。本文的容量数字描述设计目标及配置默认值，不表示本轮完成 100 用户压测。以下持久队列规则适用于站内任务与 `/api/open/v1/tasks`；标准 `/v1/images/*` 是直通路径，不提供本站任务持久化、轮询或 Webhook。当前分池/Agent 闸门见 [TASK_EXECUTION_DESIGN.md](TASK_EXECUTION_DESIGN.md)。
...
连续网络失败采用指数退避，最大 30 秒。收到 SSE 或批量快照后，只有状态、错误、产物 key 或完成时间发生变化才触发业务回调，避免无意义的界面重算。
...
Worker 使用 `global_max_concurrent_tasks`（默认 2000）作为全站图片工作量上限，站内 `user_max_concurrent_tasks` 基础默认 4，再叠加有效订阅及管理员加成。按图片数计量并跨普通任务/助手图片运行合计；多个 Worker 通过事务锁共享限制。开放任务 API 不受网页个人并发配额限制，但保留全局/线路限制。暂时无容量时保持 queued，由唤醒/延迟派发恢复。
...
每个 Worker 进程以 `WORKER_IMAGE_MEMORY_MIB`（默认 1024 MiB）控制加权图片预算；完整解码前读取尺寸，按像素及处理缓冲区预约内存。OpenAI/C2A 已完成结果转交独立的 `startOpenAIResultFetch`，在最多 5 分钟上下文中取得拉取槽后下载、保存，避免整批轮询串行等待持久化；`image_fetch_concurrency` 当前默认 8。轮询并发默认由 `WORKER_CONCURRENCY / 4` 与每路 256 MiB 图片预算推导，也可用 `WORKER_POLL_CONCURRENCY` 覆盖。不能据此忽略下载体积、内存和存储吞吐限制。
...
部署后核对实际配置：全局图片默认 2000、个人图片基础默认 4（另有有效订阅及手工加成），对话/Agent 使用独立上限。每条 Base URL 路由容量应经过上游验证；实时编辑控制新准入，已接受任务按原执行快照恢复，不应按文档默认值覆盖管理员已有配置。
