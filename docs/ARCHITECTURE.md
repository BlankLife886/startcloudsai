...
前端业务请求只访问同源 `/api/v1`。用户端、React 画布和管理端开发服务器分别监听 `3102`、`3104`、`3200`，并通过 `/api` 代理到 `localhost:8000`。生产环境的 `/canvas` 由 Vue 主站提供页面外壳，并同源嵌入 `/canvas-app/canvas` 下的 React SPA；React 仍保持独立构建和部署，嵌入时隐藏自身顶栏。
...
用户端任务等待统一使用一条账户级 SSE 和最多 100 个 ID 的批量轮询兜底，避免任务数增长时产生同等数量的连接与定时器。浏览器提交并发限制为 6，用户可排队任务默认上限为 100，实际上游并发仍由 Worker 控制。网络超时后的幂等恢复、队列去重和容量参数见 [高并发任务稳定性方案](HIGH_CONCURRENCY_TASK_STABILITY.md)。
...
sequenceDiagram
  ...
  B->>S: GET /api/v1/tasks/{id}
  S-->>B: 状态、R2 key 与站内 /api/v1/files URL
```
...
- 当前所有环境仅注册只读套餐列表 `GET /api/v1/plans`；不注册支付、订单创建、套餐购买、订阅查询、webhook 或后台补单接口。用户只能使用兑换码，管理员可做有审计记录的钱包调整。
...
提示词封面宽高作为 `prompt_library` 元数据保存，用户端据此在图片下载前计算虚拟瀑布流位置。后台上传和画廊转提示词会同步解析尺寸；历史远程 URL 由 Worker 小批量安全回填，不阻塞公开提示词接口。全站列表统一采用缩略图优先、原图按需、稳定尺寸和分级虚拟化策略，完整实现和调参边界见 [全站图片加载与瀑布流滚动性能方案](PROMPT_MASONRY_PERFORMANCE.md)。
